(function ($) {
    ...
    $('.add-article-form').submit(function(event){
        ...
        var request = $.ajax({method: form.attr('method'), url: form.attr('data-action'), data: new FormData(this), dataType: 'json'});
        ...
        request.done(function(response){
            ...
            }else if (response.status === 0 && response.field === 'authorname') {
                handleButton(button, spinner);
                handleErrors($('.authorname'), $('.authorname-error'), response.info);

            }else if (response.status === 0 && response.field === 'status') {
                handleButton(button, spinner);
                handleErrors($('.status'), $('.status-error'), response.info);

            }else if (response.status === 0 && response.field === 'authordescription') {
                handleButton(button, spinner);
                handleErrors($('.authordescription'), $('.authordescription-error'), response.info);
    ...
    $('.article-status').on('change', function() {
        var caller = $(this);
        var request = $.ajax({method: 'post', url: caller.attr('data-url'), processData: false, contentType: false, headers: {'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')},});
        request.fail(function() {
            alert('Network Error. Try Again.');
        });
    });
